...
def fit_AR_ML_gaussian_noise_SLSQP(X, phi_0_seed, phi_seed, sigma_seed):
    """ Maximum likelihood estimation of an AR model with Gaussian noise."""
    ...
    # - Cada phi_i en (-1, 1)
    # - sigma > 0
    bounds = [(None, None)]  # phi_0 sin restricciones
    bounds += [(-1 + 1e-6, 1 - 1e-6) for _ in phi_seed]  # phi_i ∈ (-1, 1)
    bounds += [(None, None)]  # sigma > 0
...
def fit_AR_ML_student_t_noise(X, phi_0_seed, phi_seed, sigma_seed, nu_seed):
    ...
    def minus_log_likelihood_AR_student_t_noise(parameters, X):
        ...
        u = residuals_AR(X, phi_0=parameters[0], phi=parameters[1:-2])
        ...
        return -np.sum(log_likelihood) # TO BE IMPLEMENTED
    ...
    info_optimization = minimize(
        lambda parameters: minus_log_likelihood_AR_student_t_noise(
            parameters, X
        ),
        parameters_seed,  
        method='nelder-mead',
        options={'xatol': 1e-8, 'disp': False}) # TO BE IMPLEMENTED
...
def fit_AR_ML_student_t_noise_SLSQP(X, phi_0_seed, phi_seed, sigma_seed, nu_seed):
    """ ML estimation of AR model with Student's t innovations."""
    from scipy.optimize import minimize
    from scipy.special import gammaln
    def minus_log_likelihood_AR_student_t_noise(parameters, X):
    ...
    # Bounds: phi_0 (free), phi (|phi| < 1), sigma > 0, nu > 2
    bounds = [(None, None)]                        # phi_0
    bounds += [(-1 + 1e-6, 1 - 1e-6)] * len(phi_seed)  # phi
    bounds += [(1.0e-8, None)]                      # sigma
    bounds += [(2 + 1e-6, None)]                  # nu
...
def fit_AR_GARCH_ML_student_t_noise(
    ...
    return phi_0, phi, kappa, alpha, beta, nu, info_optimization  # TO BE IMPLEMENTED
